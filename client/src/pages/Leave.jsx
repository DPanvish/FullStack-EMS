    ...
    <div className="animate-fade-in">
      ...
      {!isAdmin && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8">
          {leaveStats.map((s) => (
            <div key={s.label} className="group leave-stat-card">
              <div className="leave-stat-accent" />
              <div className="leave-stat-icon-box">
                <s.icon className="leave-stat-icon" />
              </div>
              <div>
                <p className="leave-stat-label">{s.label}</p>
                <p className="leave-stat-value">
                  {s.value} <span className="leave-stat-unit">taken</span>
                </p>
              </div>
            </div>
          ))}
